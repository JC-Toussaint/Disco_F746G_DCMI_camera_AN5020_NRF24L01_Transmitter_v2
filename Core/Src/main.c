...
#define CAMERA
	...
	/*##-DEBUG- Direct Access to SDRAM ############################*/
	// Writing in the video memory shared with the STM32F7 memory
	// each pixel is coded in 16 bits, we can put 2 pixels by word of 32 bits (= 2 bytes)
	// As the addressing is in 32 bits, the size of the image in 32 bits words (= 4 bytes)
	// is : Im_size=(480*272)*2 / 4
#define no__DEBUG__
#ifdef __DEBUG__
	LCD_DisplayOn();
	...
	hal_status = HAL_DCMI_Start_DMA(&hdcmi, DCMI_MODE_SNAPSHOT, (uint32_t)CAMERA_FRAME_BUFFER, Im_size);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOF, NRF_CSN_Pin|NRF_CE_Pin, GPIO_PIN_RESET);
